...
class AI(object):
    def __init__(self, local_chessboard_size, color, time_out, evaluation=random_base_line):
        global chessboard_size
        chessboard_size = self.chessboard_size = local_chessboard_size
        self.color = color  # 黑或白
        self.time_out = time_out * 0.94  # 防止时间波动导致超时
        self.candidate_list = []
    ...
    def go(self, chessboard):
        ...
        if len(self.candidate_list) == 0 or len(self.candidate_list) == 1:  # 不要self.rounds<=5， 趁着第一回合jit一下
            ...
            return
        if self.rounds >= 64 - 9:
            # print(f"在{self.rounds}回合启动ab决战搜索！")
            value, decision = iterative_deepening_search(chessboard, self.color, self.time_out, self.rounds)
            if decision is not None:
                self.execute_decision(decision)
        else:
            self.execute_decision(max(self.candidate_list,
                                      key=lambda x: self.evaluation(
                                          updated_chessboard(chessboard, self.color, np.array(x)),
                                          self.color, self.rounds)))
        self.rounds += 2
...
def iterative_deepening_search(chessboard, current_color, time_out, rounds, memory_out=1048576):
    """
